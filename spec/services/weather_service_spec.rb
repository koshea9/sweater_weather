...
RSpec.describe WeatherService do
  it 'returns weather data given valide latitude and longitude paramters' do
    VCR.use_cassette('Denver weather') do
      lat = 39.738453
      lng = -104.984853
      response = WeatherService.get_weather_data(lat, lng)
      ...
      expect(response[:current][:weather]).to be_a(Hash)
      expect(response[:current][:weather]).to have_key(:description)
      expect(response[:current][:weather][:description]).to be_a(String)
      # icon
      expect(response[:current][:weather]).to have_key(:icon)
      expect(response[:current][:weather][:icon]).to be_a(String)
      # daily_weather
      expect(response).to have_key(:daily)
      expect(response[:daily]).to be_a(Array)
      expect(response[:daily].count).to eq(8)
      # datetime
      expect(response[:daily]).to have_key(:dt)
      ...
      # conditions
      expect(response[:daily][0][:weather]).to have_key(:description)
      expect(response[:daily][0][:weather][:description]).to be_a(String)
      # icon
      expect(response[:daily][0][:weather]).to have_key(:icon)
      expect(response[:daily][0][:weather][:icon]).to be_a(String)
      # hourly_weather
      ...
      # conditions
      expect(response[:hourly][0][:weather]).to have_key(:description)
      expect(response[:hourly][0][:weather][:description]).to be_a(String)
      # icon
      expect(response[:hourly][0][:weather]).to have_key(:icon)
      expect(response[:hourly][0][:weather][:icon]).to be_a(String)
    end
  end
end
